...
public class ProductController {
    ...
    */
    ...
    public ResponseEntity<ProductCreateResponse> createProduct(
        ...
        URI location = URI.create("/products/" + newId);
        // Location 헤더에 방금 생성한 리소스의 URI 넣기.
        // 문자열 주소를 URI 객체로! (이건 Location 헤더에 넣을 용도)

        // message + 생성된 productId 반환 (JSON 바디)
        ProductCreateResponse productCreateResponse =
                new ProductCreateResponse(newId, "상품 등록 완료");

        // 201 Created + Location + JSON 바디
        return ResponseEntity.created(location).body(productCreateResponse);

        // ProductCreateResponse DTO 만들기 싫다면?
        // return ResponseEntity.created(URI.create("/products/" + newId))
        //                      .body("상품 등록 성공 [id = " + newID + "]"); 가능.
    }
